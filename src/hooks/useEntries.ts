...
export function useEntries() {
  ...
  const addEntry = useCallback((
    ...
    setEntries(prev => [{ id: uuidv4(), text, category, categoryMeta, tags, imageIds, location, createdAt }, ...prev])
  }, [])

  const updateEntry = useCallback((
    id: string, text: string, category: CategoryId | null, categoryMeta: CategoryMeta,
    tags: string[], imageIds: string[], location: string,
  ) => {
    setEntries(prev => prev.map(e => e.id === id ? { ...e, text, category, categoryMeta, tags, imageIds, location } : e))
  }, [])
