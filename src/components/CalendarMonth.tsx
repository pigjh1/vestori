...
export function CalendarMonth({ entries, onDayClick, selectedDateKey }: CalendarMonthProps) {
  ...
  return (
    <div className="bg-white border border-paper-border rounded-sm mb-5">
      <div className="flex items-center justify-between px-4 py-3 border-b border-paper-border">
        <button onClick={prevMonth} className="text-ink-faint hover:text-ink cursor-pointer bg-none border-none px-2 py-1 text-[14px]">←</button>
        <span className="font-serif text-[14px] italic text-ink-muted">{year}년 {MONTHS[month]}</span>
        <button onClick={nextMonth} className="text-ink-faint hover:text-ink cursor-pointer bg-none border-none px-2 py-1 text-[14px]">→</button>
      </div>
      <div className="p-3">
        <div className="grid grid-cols-7 mb-1">
          {WEEKDAYS.map(d => <div key={d} className="text-center font-sans text-[10px] text-ink-faint py-1">{d}</div>)}
        </div>
            ...
            return (
              <button key={key} onClick={() => !future && onDayClick(key)}
                className={`flex flex-col items-center py-1 rounded-sm transition-all cursor-pointer bg-none border-none
                  ${selected ? 'bg-accent-pale' : 'hover:bg-paper-warm'} ${future ? 'opacity-25 cursor-default' : ''}`}>
                <span className={`font-sans text-[12px] leading-none ${today ? 'text-accent font-medium' : 'text-ink-muted'}`}>
                  {day.getDate()}
                </span>
