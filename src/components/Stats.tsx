  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-paper-card border border-paper-border rounded-sm p-4">
        <p className="text-xs text-ink-faint mb-3 tracking-wide">최근 30일</p>
      ...
      {stats.catCount.length > 0 && (
        <div className="bg-paper-card border border-paper-border rounded-sm p-4">
          <p className="text-xs text-ink-faint mb-3 tracking-wide">카테고리</p>
          <div className="flex flex-col gap-2">
            {stats.catCount.map(({ id, label, count }) => (
              <div key={id} className="flex items-center gap-3">
                <span className="text-xs text-ink-muted w-12">{label}</span>
      ...
      <div className="bg-paper-card border border-paper-border rounded-sm p-4">
        <p className="text-xs text-ink-faint mb-3 tracking-wide">요일별 기록</p>
        <div className="flex gap-2 items-end h-14">
          {stats.dayMap.map((count, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-1">
              ...
              <span className="text-xs text-ink-faint">{stats.DAYS[i]}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      {stats.topTags.length > 0 && (
        <div className="bg-paper-card border border-paper-border rounded-sm p-4">
          <p className="text-xs text-ink-faint mb-3 tracking-wide">자주 쓴 태그</p>
          <div className="flex flex-wrap gap-2">
            {stats.topTags.map(([tag, count]) => (
              <span key={tag} className="text-xs px-3 py-1.5 rounded-sm bg-ink/8 text-ink border border-ink/15">
                #{tag} <span className="text-accent/60 text-sm">{count}</span>
              </span>
            ))}
          </div>
        </div>
