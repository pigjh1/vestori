                ...
                return (
                  <td key={m} style={{width: '20px', height: '20px', padding: '2px'}}>
                    <div className="w-full h-full rounded-[2px] cursor-pointer transition-all hover:ring-2 hover:ring-accent relative"
                      ...
                      title={dateKey}>
                      {avg !== undefined && <span className="absolute inset-0 flex items-center justify-center text-[6px] font-bold text-white" style={{textShadow: '1px 1px 2px rgba(0,0,0,0.3)'}}>{avg.toFixed(1)}</span>}
                    </div>
                  </td>
                )
              })}
                ...
                {Array.from({length: monthDays}, (_, d) => {
                  ...
                  return (
                    <div key={d} className="aspect-square">
                      <div className="w-full h-full rounded-sm cursor-pointer transition-all hover:ring-2 hover:ring-accent flex items-center justify-center font-sans text-[9px] text-white relative"
                        style={{ background: bg, opacity }}
                        onClick={() => openInput(dateKey)}
                        onMouseEnter={() => setHovered(dateKey)}
                        onMouseLeave={() => setHovered(null)}
                        title={dateKey}>
                        <span>{d + 1}</span>
                        {avg !== undefined && <span className="absolute text-[7px] font-bold" style={{textShadow: '0.5px 0.5px 1px rgba(0,0,0,0.3)'}}>{avg.toFixed(1)}</span>}
                      </div>
                    </div>
                  )
                })}
  ...
  return (
    <div>
      <div className="flex items-baseline gap-3 mb-5">
        ...
        {avgOverall && <span className="font-sans text-[12px] text-ink-faint">평균 <span className="text-accent">{avgOverall}</span></span>}
      </div>
      ...
      {todayEntries.length === 0 ? (
        ...
        <div onClick={() => openInput(today)}
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="font-sans text-[11px] text-ink-faint mb-0.5">오늘 ({todayEntries.length}회)</p>
        ...
        <div className="fixed inset-0 z-40 flex items-end sm:items-center justify-center p-4">
          ...
          <div className="relative bg-paper border border-paper-border rounded-sm w-full max-w-sm shadow-xl z-10 overflow-hidden">
            ...
            <div className="px-6 pt-5 pb-6">
              ...
              <div className="mb-4">
                <label className="font-sans text-[11px] text-ink-faint">시간</label>
                <input type="time" value={inputTime} onChange={e => setInputTime(e.target.value)}
                  className="w-full font-sans text-ink bg-paper-warm border border-paper-border rounded-sm px-3 py-2 outline-none focus:border-accent-light transition-colors mt-1" />
              </div>
              ...
              <div className="flex gap-2">
                ...
                {recordsByDate[selectedDate] && recordsByDate[selectedDate].length > 0 && (
                  <button onClick={() => { deleteDay(selectedDate); setSelectedDate(null) }}
                    className="font-sans text-[13px] text-ink-faint border border-paper-border px-3 py-2 rounded-sm cursor-pointer hover:border-accent-light transition-colors">이날 삭제</button>
                )}
