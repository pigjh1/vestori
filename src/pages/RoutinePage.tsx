      ...
      <div className="bg-white border border-paper-border rounded-sm p-4 mb-6 relative overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="text-right">
            ...
            <p className="font-sans text-[11px] text-ink-faint">/ {habits.length}개 완료</p>
          </div>
        </div>
        {habits.length > 0 && (
      ...
      <div className="flex flex-col gap-0 mb-4">
        {habits.length === 0 && !adding && (
          <div className="text-center py-12">
            ...
            <p className="font-body text-sm font-light text-ink-faint italic leading-[1.7]">아직 루틴이 없어요.<br />아래 버튼으로 추가해보세요.</p>
          </div>
        )}
            ...
            <div key={habit.id}
              ...
              <button onClick={() => toggle(today, habit.id)}
                className={`w-5 h-5 rounded-sm border-2 flex items-center justify-center flex-shrink-0 transition-all cursor-pointer
                  ${done ? 'bg-accent border-accent' : 'border-paper-border hover:border-accent-light bg-transparent'}`}>
                {done && (
              ...
              <span className={`font-body text-[15px] font-light flex-1 transition-all ${done ? 'text-ink-faint line-through' : 'text-ink'}`}>
                {habit.name}
              </span>

              {habit.streak > 0 && (
                <span className="font-sans text-[10px] text-accent-light opacity-0 group-hover:opacity-100 transition-opacity">
                  {habit.streak}일 연속
                </span>
              )}

              <button onClick={() => deleteHabit(habit.id)}
                className="opacity-0 group-hover:opacity-100 transition-opacity text-ink-faint hover:text-accent cursor-pointer bg-none border-none text-[16px] flex-shrink-0">
                ×
              </button>
            </div>
          )
      ...
      {adding ? (
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleAdd} disabled={!newHabit.trim()}
            className="font-sans text-[13px] bg-ink text-white px-4 py-2 rounded-sm hover:bg-accent transition-colors cursor-pointer disabled:opacity-40">추가</button>
        ...
        <button onClick={() => setAdding(true)}
          className="w-full py-3 border border-dashed border-paper-border rounded-sm font-sans text-[13px] text-ink-faint hover:text-ink hover:border-accent-light transition-all cursor-pointer">
          + 루틴 추가
        </button>
      )}
...
function WeekSummary({ habits, checks }: { habits: { id: string }[], checks: Record<string, Record<string, boolean>> }) {
  ...
  return (
    <div className="mt-8 pt-6 border-t border-paper-border">
      <p className="font-sans text-[11px] text-ink-faint tracking-wide mb-4">최근 7일</p>
